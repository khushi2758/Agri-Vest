...
import { TrendingUp, TrendingDown, Calendar, Clock, Wallet, ArrowUpRight, Search, Filter, ChevronDown, MoreVertical, Plus, Paperclip, CheckCircle2, ListTodo } from "lucide-react";
const STOCKS = [{
  id: "AGRI-1001",
  name: "Sundance Corn",
  shares: "250",
...
export default function StockDetailsPage() {
  const [selectedStock, setSelectedStock] = useState(STOCKS[2]);
  return <div className="min-h-screen bg-[#f7f9f2] font-sans pb-20 selection:bg-[#c8e639] selection:text-black">
      <NavBar />
      ...
      <div className="mx-auto max-w-350 px-6 pt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between h-48 relative">
            ...
            <div className="flex items-end gap-2 mt-4 h-12 w-full">
              {[30, 40, 20, 50, 45, 70, 85].map((h, i) => <div key={i} className="w-full bg-[#c8e639]/40 rounded-t-sm" style={{
              height: `${h}%`
            }}></div>)}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between h-48">
            ...
            <div className="mt-4 relative h-12 w-full">
               <svg viewBox="0 0 100 30" className="w-full h-full overflow-visible">
                 ...
                 {[[0, 25], [15, 20], [30, 22], [45, 15], [60, 18], [75, 10], [90, 12], [100, 5]].map((pt, i) => <circle key={i} cx={pt[0]} cy={pt[1]} r="2.5" fill="white" stroke="#c8e639" strokeWidth="1.5" />)}
               </svg>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input type="text" placeholder="Enter stock ticker #" className="w-full bg-white border border-gray-200 text-sm font-medium rounded-xl pl-9 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#c8e639] focus:border-transparent" />
          </div>
        </div>
        ...
        <div className="bg-[#1b2620] rounded-[2.5rem] p-4 flex flex-col lg:flex-row gap-4 shadow-2xl overflow-hidden min-h-150">
          ...
          <div className="w-full lg:w-100 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2 mt-4 custom-scrollbar">
              ...
              return <button key={stock.id} onClick={() => setSelectedStock(stock)} className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all duration-300 text-left ${isSelected ? "bg-[#c8e639] text-[#1b2620] shadow-md scale-[1.02]" : "hover:bg-[#2c3d33] text-white/80"}`}>
                    <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md ${isSelected ? "bg-[#1b2620]/10 text-[#1b2620]" : "bg-white/10 text-white/60"}`}>
                        {stock.status}
                      </span>
                    </div>
                  </button>;
