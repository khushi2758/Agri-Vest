...
const VOICE_GUIDE_ROLES = ["farmer", "landowner"];
...
export default function HelpTourButton({
  ...
  const hasSpeechContent = !!description || (speechSections && speechSections.length > 0);
  const isVoiceEligibleRole = !!user?.role && VOICE_GUIDE_ROLES.includes(user.role);
  const hasVoice = hasSpeechContent && isVoiceEligibleRole;
        ...
        {reading && (
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-gray-500">Stop narration.</p>
            </div>
          </button>
        )}
        <button
